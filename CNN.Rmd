...
```{r}
# Default Model
model_function_binary <- function(learning_rate = 0.001) {
...
# RMSprop
model_function_rmsprop <- function(learning_rate = 0.001) {
...
```{r noise_02_categorical_crossentropy}
...
# Binds training set with noise to original set,
noise_x_train = abind(x, noise_x, along = 1)

# New shuffle indicies
set.seed(68)
noise_shuf_ind = sample(1:dim(x)[1], dim(x)[1]*0.8)

# Shuffle new training dataset
noise_x_train = noise_x[noise_shuf_ind, , ,]
...
noise_y <- abind(yy, yy, along = 1)
noise_y_train <- noise_y[noise_shuf_ind, ]
```
...
#save_model_tf(model_catent, "cnn_catent_cweights_noise02")
```
...
plot(hist, main = "Learning Curve for Model with Binary Cross Entropy")

# save_model_tf(model_binary, "cnn_binary_noise02")
```

```{r noise_02_rmsprop_train}
...
save_model_tf(model_rmsprop, "cnn_rmsprop_noise02")
```
